...
start_sector = 100032 # **** ATTENTION -> THIS HAS TO BE CHANGED ACCORDING TO YOUR PARTITION TABLE ****
...
def write_chunks_to_device():
    chunks = sorted([f for f in os.listdir(output_dir) if f.endswith(".bin")], key=lambda x: int(x.replace('system_chunk', '').replace('.bin', '')))
