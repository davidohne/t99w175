...
vid = "13fb"
pid = "eafd"
input_file = "modem.bin" # Location of the MODEM image file on your filesystem
...
output_dir = "modem_write_chunks"  # Directory to store splitted chunks
...
def split_file():
    with open(input_file, 'rb') as file:
        ...
        while True:
            ...
            chunk_file_path = os.path.join(output_dir, f"modem_chunk{chunk_index}.bin")
...
def write_chunks_to_device():
    chunks = sorted([f for f in os.listdir(output_dir) if f.startswith("modem_chunk")], key=lambda x: int(x.replace('modem_chunk', '').replace('.bin', '')))
