...
vid = "13fb"
pid = "eafd"
input_file = "fsg.bin" # Location of the FSG image file on your filesystem
start_sector = 39744  # **** ATTENTION -> THIS HAS TO BE CHANGED ACCORDING TO YOUR PARTITION TABLE ****
sectors_per_chunk = 2048
sector_size = 4096  
chunk_size = sectors_per_chunk * sector_size  
output_dir = "fsg_write_chunks"  # Directory to store splitted chunks
...
def split_file():
    with open(input_file, 'rb') as file:
        ...
        while True:
            ...
            chunk_file_path = os.path.join(output_dir, f"fsg_chunk{chunk_index}.bin")
...
def write_chunks_to_device():
    chunks = sorted([f for f in os.listdir(output_dir) if f.startswith("fsg_chunk")], key=lambda x: int(x.replace('fsg_chunk', '').replace('.bin', '')))
