...
total_sectors = 22720  # **** ATTENTION -> THIS HAS TO BE CHANGED ACCORDING TO YOUR PARTITION TABLE ****
sectors_per_read = 2048  # Number of sectors per read attempt

if not os.path.exists(output_dir):
    os.makedirs(output_dir)

def run_edl_command(start_sector, num_sectors, chunk_index):
    output_file = f"{output_dir}/modem_chunk{chunk_index}.bin"
    command = f"python3.12 edl rs {start_sector} {num_sectors} {output_file} --vid={vid} --pid={pid}"
